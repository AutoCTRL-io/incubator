...
**Active modules:** `wifi_module`, `webserver_module` (via `wifi_setup`/`wifi_loop`, `webserver_setup`/`webserver_loop`)  
**Served:** `/`, `/wifi`, static assets (`/style.css`, `/app.js`)  
**Not yet in use:** `ota_module`, `appstate_module`, `core_module`, `dht_module`, `stepper_module`, `ws_module`, `profiles_module`, `webassets_module` (full UI)
...
- **appstate_module:** NVS load/save, global `process` struct, helpers; `appstate_setup`/`appstate_loop`.
...
- **wifi_module:** AP/STA, NVS credentials; `wifi_setup`/`wifi_loop` (prints WiFi IP every 1s).
- **ota_module:** OTA updates; `ota_setup`/`ota_loop`.
- **webassets_module:** HTML/CSS/JS blobs; `webassets_setup`/`webassets_loop`.

## Recent changes

- All modules renamed to `x_module.h`/`x_module.cpp` with `x_setup()` and `x_loop()`; incubator.ino calls only `wifi_setup`/`wifi_loop`, `webserver_setup`/`webserver_loop`.
- WiFi logic in `wifi_module`; `wifi_loop()` prints WiFi (STA) IP every 1 second to serial.
